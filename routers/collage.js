...
router.route('/')
    ...
    .post(verifyAccessToken, isAdmin, async(req, res, next) => {
        try {
            const data = new Collage(req.body);
...
router.route('/:id')
    ...
    .delete(verifyAccessToken, isAdmin, async(req, res, next) => {
        try {
            await Collage.findByIdAndUpdate(req.params.id, { status: 0 }, { new: true });
...
router.get('/count', verifyAccessToken, isAdmin, async(req, res, next) => {
    try {
        const Collage = await Collage.count(req.query)
        res.json({ count: Collage });
    } catch (error) {
        next(error)
    }
});
